...
public class GPUGemsTerrain : MonoBehaviour {
    ...
    [Range (8, 32)] public int resolution = 32;
    ...
    public float riverFrequency, riverAmplitude ,riverFalloff;

    [System.Serializable]
    public class Octave {
        public float frequency = 1, amplitude = 1;
    }
    void Start () {
        //GenerateMesh ();
    }

    public float Density (int x, int y, int z, int width, int height, int depth, FractalNoise fractal) {
        ...
        for (int i = 0; i < octaves.Count; i++) {
            ...
            density += fractal.Sample3D (ws * frequency) * amplitude;
        }
    ...
    public float RiverDensity (int x, int y, int z, int width, int height, int depth, FractalNoise fractal) {
        float u = x / (width - 1.0f);
        float v = y / (height - 1.0f);
        float w = z / (depth - 1.0f);
        float n = fractal.Sample3D (u * riverFrequency, v * riverFrequency, w * riverFrequency) * riverAmplitude;
        float d = 1 - (Mathf.Abs (n - 0.5f) * 2);
        d = Mathf.Pow (d, riverFalloff);
        return d;
    }
    ...
    public void GenerateMesh () {
        ...
        FractalNoise fractal = new FractalNoise (selectedInoise, 1, 1.0f);
        ...
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                for (int z = 0; z < depth; z++) {
                    float density = Density (x, y, z, width, height, depth, fractal);
                    float riverDensity = RiverDensity (x, y, z, width, height, depth, fractal);
                    voxels[x, y, z] = Mathf.Min (density, riverDensity);
                }
            }
        }
